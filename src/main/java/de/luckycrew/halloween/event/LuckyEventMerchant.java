...
public class LuckyEventMerchant extends LuckyEventCustom {
	...
	public LuckyEventMerchant() {
		super("Merchant", 2);
		for (Item item : HalloweenItems.scarecrow) {
			armor.add(item);
		}
		for (Item item : HalloweenItems.slender) {
			armor.add(item);
		}
		for (Item item : HalloweenItems.witch) {
			armor.add(item);
		}
		for (Item item : HalloweenItems.zombie) {
			armor.add(item);
		}
		for (Item item : HalloweenItems.clown) {
			armor.add(item);
		}
	}
	...
	private ArrayList<MerchantRecipe> getRecipes() {
		...
		for (int i = 0; i < MathUtil.getRandomNumberInRange(5, 15); i++) {
			...
			case 0:
				...
				re = new MerchantRecipe(diamond, ItemStackUtil.from(HalloweenItems.killerknive));
				break;
			case 1:
				goldingot.stackSize = MathUtil.getRandomNumberInRange(55, 64);
				re = new MerchantRecipe(goldingot, goldingot, ItemStackUtil.from(HalloweenItems.killerknive));
				break;
			case 2:
